...
import { FaBox, FaThList, FaCogs, FaDatabase, FaUsers, FaImage, FaNewspaper } from "react-icons/fa";
import "./../../public/styles/css/dashboard.css"; 
...
const AdminDashboard = () => {
  ...
  const handleSectionClick = (index) => {
    setOpenSection(openSection === index ? null : index);  // переключение раскрытого раздела
  };
      ...
      <aside className="sidebar">
        ...
        <ul>
          {sections.map((section, index) => (
            <li key={index}>
              <div onClick={() => handleSectionClick(index)} className="sidebar-item">
                {section.icon} <span>{section.name}</span>
                <span className="arrow">{openSection === index ? '▲' : '▼'}</span> {/* Стрелочка */}
              </div>
              {openSection === index && (
                <ul className="crud-menu">
                  <li><Link to={`${section.path}/`}>View List</Link></li>
                  <li><Link to={`${section.path}/add`}>Create New</Link></li>
                  <li><Link to={`${section.path}/update`}>Update</Link></li>
                  <li><Link to={`${section.path}/delete`}>Delete</Link></li>
                </ul>
              )}
            </li>
          ))}
